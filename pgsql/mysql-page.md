...
问题：MySql的QueryCache想要命中的话，难道真的需要“要求查询语句与之前的一模一样，包括大小写必须一致、不能增减空格等等。”？
⁃是的，包括comment都必须是一样的，因为mysql使用简单的hash来确保的。
...
⁃SELECT ip_country FROM geoip WHERE INET_ATON('174.36.207.186') BETWEEN begin_ip_num AND end_ip_num LIMIT 1;
```